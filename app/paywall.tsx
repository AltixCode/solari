...
export default function Paywall() {
  ...
  // Same 700pt breakpoint `useTabletColumn` and the app's own `Text`
  // component already use for "is this a tablet". Below it the two halves of
  // the panel stack top/bottom with a horizontal rule; at or above it they
  // sit side by side with a vertical rule.
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Vertical divider split: one bordered panel, cut in two by a single
            rule. The free/ad-supported half sits in a muted treatment and the
            unlock half in the full-emphasis one — not two separate cards, and
            not a toggle. Below 700pt (the same breakpoint `useTabletColumn`
            and `Text` already use for "is this a tablet") the rule turns
            horizontal and the halves stack instead of sitting side by side. */}
        <View
          testID="paywallSplitPanel"
          style={{
        ...
        >
          ...
          >
            <Text variant="micro" tone="accent">
              {t("paywallUnlockLabel")}
            </Text>
            <View style={{ marginTop: spacing.base, gap: spacing.base }}>
